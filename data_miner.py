...
class Gui(GuiBase):
    ...
    def _init_api_client(self):
        api_id = (
            self._config.get("API", "id")
            if self._config.has_option("API", "id")
            else None
        )
        api_key = (
            self._config.get("API", "key")
            if self._config.has_option("API", "key")
            else None
        )
        if api_id and api_key:
            ...
            if self._config.has_option("API", "endpoint"):
                ...
                if endpoint:
                    self._api_client._endpoint = endpoint
    ...
    def _auth_submit(self, event=None):
        ...
        threading.Thread(
            target=self._auth_check_credentials, args=(api_id, api_key)
        ).start()
    ...
    def _build_auth_frame(self):
        if self._auth is None:
            ...
            ttk.Label(self._auth["frame"], text="Api ID").grid(
                row=0, column=0, sticky="SE"
            )
            self._auth["api_id_entry"] = ttk.Entry(
                self._auth["frame"], width=40, textvariable=self._auth["api_id_var"]
            )
            self._auth["api_id_entry"].grid(row=0, column=1, sticky="SW", padx=5)
            ttk.Label(self._auth["frame"], text="Api Key").grid(
                row=1, column=0, sticky="E", pady=5
            )
            api_key_entry = ttk.Entry(
                self._auth["frame"], width=40, textvariable=self._auth["api_key_var"]
            )
            ...
            self._auth["submit_btn"] = ttk.Button(
                self._auth["frame"], text="Confirm", command=self._auth_submit
            )
    ...
    def _build_main_frame(self):
        if self._main is None:
            self._main = {"frame": ttk.Frame(self._window, padding=(5, 10, 5, 5))}

            self._main["frame"].grid(row=0, column=0, sticky="NSEW")

            self._main["paned_window"] = ttk.PanedWindow(
                self._main["frame"], orient=tk.VERTICAL
            )
            ...
            ttk.Separator(frame, orient="vertical").pack(
                side=tk.LEFT, fill="y", padx=5, pady=3
            )
            self._main["btn_execute"] = ttk.Button(
                frame, text="Execute", command=self._execute
            )
            ...
            self._main["btn_auto_save_output_folder"] = ttk.Label(
                frame,
                text=(
                    "(" + self._auto_save_folder + ")" if self._auto_save.get() else ""
                ),
            )
    ...
    def _auto_save_output_init(self):
        ...
        self._auto_save_folder = (
            self._config.get("OUTPUT", "auto_save_folder")
    ...
    def _build_menu(self):
        ...
        if not self._main.get("menu_bar"):
            ...
            input_menu.add_command(
                label="Save", underline=0, accelerator="Ctrl+S", command=save_callback
            )
            input_menu.add_command(
                label="Save As",
                underline=5,
            ...
            self._main["menu_bar"].add_cascade(
                label="Input", underline=0, menu=input_menu
            )
            ...
            try:
                ...
                for level1 in os.listdir(path):
                    ...
                    if os.path.isfile(entry):
                        ...
                        regex = re.compile(
                            rf"^{level1}(\s*-\s*)?|\.yaml$", re.IGNORECASE
                        )
                        ...
                        for level2 in entries:
                            ...
                            submenu.add_command(
                                ...
                                command=functools.partial(
                                    ...
                                    True,
                                    os.path.join(entry, level2),
                                    True,
                                ),
                            )
                        samples_menu.add_cascade(label=level1, menu=submenu)
                self._main["menu_bar"].add_cascade(
                    label="Samples", underline=1, menu=samples_menu
                )
            except Exception:
                pass
            ...
            session_menu.add_command(
                label="Logout", underline=0, accelerator="Ctrl+L", command=self._logout
            )
            session_menu.add_command(
                label="Quit", underline=0, accelerator="Ctrl+Q", command=self._on_close
            )
            self._main["menu_bar"].add_cascade(
                label="Session", underline=0, menu=session_menu
            )
        ...
        GuiBase.bind_ci(
            self._window, True, modifier="Control", letter="s", callback=save_callback
        )
        GuiBase.bind_ci(
            self._window,
            True,
        ...
        GuiBase.bind_ci(
            self._window, True, modifier="Control", letter="l", callback=self._logout
        )
        GuiBase.bind_ci(
            self._window, True, modifier="Control", letter="q", callback=self._on_close
        )
        ...
        GuiBase.bind_ci(
            self._window, True, modifier="Control", letter="e", callback=self._execute
        )
    ...
    def _input_text_modified_event_toggle(self, state: bool = True):
        ...
        if state:
            if self._main.get("input_text_modified_unbind") is None:
                self._main["input_text_modified_unbind"] = self._main["input"].bind(
                    "<<TextModified>>", self._main["input_text_modified_callback"]
                )
    ...
    def _build_console_frame(self):
        main_frame = ttk.LabelFrame(
            self._main["paned_window"], text="Console", padding=5
        )
            ...
            if rows:
                if from_btn:
                    ...
                    file = (
                        self._auto_save_folder
                        + "/"
                        + str(self._operation.get_name()).lower()
                        + "_"
    ...
    def _save_input(self, save_as: bool = False, init: bool = True, *args):
        ...
        if init:
            ...
            self.toggle_state(
                self._main[
                    "menu_input_save_as_cmd" if save_as else "menu_input_save_cmd"
                ]
            )
        ...
        self.toggle_state(self._main["menu_input_close_cmd"], True)
        self.toggle_state(
            self._main["menu_input_save_as_cmd" if save_as else "menu_input_save_cmd"],
            True,
        )
        self.toggle_state(self._main["input"], True)

    def _open_input_file(
        self, init: bool = True, file_path: str = None, anonymous_mode: bool = False
    ):
        """
        ...
        if init:
            ...
            self.toggle_state(self._main["input"])
            threading.Thread(
                target=self._open_input_file, args=[False, file_path, anonymous_mode]
            ).start()
            return
    ...
    def _update_displayed_file_name(self, changed: bool = False, *args):
        ...
        if opened_file:
            ...
            self._window.title(
                "{} - v{}{}".format(cons.NAME, cons.VERSION, " - *" if changed else "")
            )
    ...
    def _execute(self, init: bool = True):
        ...
        if (
            ...
            or not self._operation.is_paused()
