...
SETTINGS = {
    ...
    "Formulas": {"isValid": validation.data_univ_formulas, "required": True},
    "Include Names": {"field": "includeNames", "isValid": misc.is_bool},
    "Currency": {"field": "currency", "isValid": validation.currency},
    "Region": {"field": "region", "isValid": misc.is_str},
    "Ignore Errors": {"field": "ignoreErrors", "isValid": misc.is_bool},
}
...
UNIVERSE_SETTINGS = {
    ...
    "End Date": {"isValid": validation.date, "transform": transform.date},
    "Frequency": {
        "isValid": functools.partial(
            validation.from_mapping,
            mapping={item["label"]: item["value"] for item in cons.FREQ[1:]},
        )
    },
    "Universe": {
        "field": "universe",
