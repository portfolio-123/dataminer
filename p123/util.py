...
def generate_params(
    *, data: dict, settings, api_client: Client, logger: logging.Logger
):
    """
    ...
    for entry in data.values():
        ...
        if "transform" in meta_info:
            value = meta_info["transform"](
                value=entry["value"], settings=settings, api_client=api_client
            )
        if value is None:
            return
        target = params
        ...
        if (
            meta_info["field"] == "rules"
            and misc.is_list(target.get(meta_info["field"]))
            and misc.is_list(value)
        ):
...
def process_screen_rolling_backtest_result(json: dict, start_dt, end_dt, precision):
    ...
    data.append(
        round(
            statistics.fmean(float(item[5]) for item in json["rows"][0:13]), precision
        )
        if length >= 13
        else None
    )
    data.append(
        round(
            statistics.fmean(float(item[5]) for item in json["rows"][0:65]), precision
        )
        if length >= 65
        else None
    )
        ...
        round(
            (
                statistics.geometric_mean(
                    float(item[5]) / 100 + 1 for item in json["rows"]
                )
                - 1
            )
            * 100,
    ...
    data.append(
        round(
            (
                statistics.geometric_mean(
                    float(item[6]) / 100 + 1 for item in json["rows"]
                )
                - 1
            )
            * 100,
    ...
    data.append(
        round(
            (
                statistics.geometric_mean(
                    float(item[5]) / 100 + 1 for item in json["rows"][0:13]
                )
                - 1
            )
            * 100,
    ...
    data.append(
        round(
            (
                statistics.geometric_mean(
                    float(item[5]) / 100 + 1 for item in json["rows"][0:65]
                )
                - 1
            )
            * 100,
    ...
    data.append(
        round(
            statistics.fmean(float(item[4]) for item in json["rows"][0:65]), precision
        )
        if length >= 65
        else None
    )
...
def process_rank_perf_result(json: dict, rows: list, port_mode: bool, precision):
    ...
    idx += 1
    rows[idx].append(
        misc.round_or_none(json["stats"][port_or_bench]["annualized_return"], precision)
    )
    ...
    rows[idx].append(
        misc.round_or_none(
            json["stats"]["port"]["annualized_return"]
            - json["stats"]["bench"]["annualized_return"],
            precision,
        )
        if port_mode
        else None
    ...
    idx += 1
    rows[idx].append(
        misc.round_or_none(json["stats"][port_or_bench]["total_return"], precision)
    )

    # % of periods strategy outperforms
    idx += 1
    rows[idx].append(
        round(per_outperform_cnt / len(json["results"]["rows"]) * 100, precision)
        if port_mode
        else None
    )
    ...
    idx += 1
    rows[idx].append(
        misc.round_or_none(json["results"]["upMarkets"][10], precision)
        if port_mode
        else None
    )
    ...
    idx += 1
    rows[idx].append(
        misc.round_or_none(json["results"]["downMarkets"][10], precision)
        if port_mode
        else None
    )
    ...
    idx += 1
    rows[idx].append(
        misc.round_or_none(json["stats"][port_or_bench].get("sharpe_ratio"), precision)
    )

    # Sortino
    idx += 1
    rows[idx].append(
        misc.round_or_none(json["stats"][port_or_bench].get("sortino_ratio"), precision)
    )

    # StdDev
    idx += 1
    rows[idx].append(
        misc.round_or_none(json["stats"][port_or_bench].get("standard_dev"), precision)
    )

    # Max Drawdown
    idx += 1
    rows[idx].append(
        misc.round_or_none(json["stats"][port_or_bench].get("max_drawdown"), precision)
    )

    # Beta
    idx += 1
    rows[idx].append(
        misc.round_or_none(json["stats"]["beta"], precision)
        if port_mode and json["stats"].get("beta")
        else None
    )
    ...
    idx += 1
    rows[idx].append(
        misc.round_or_none(json["stats"]["alpha"], precision)
        if port_mode and json["stats"].get("alpha")
        else None
    )
    ...
    idx += 1
    rows[idx].append(
        misc.round_or_none(json["results"]["average"][4], precision)
        if port_mode
        else None
    )
...
def validate_main(*, settings, logger: logging.Logger):
    for prop, meta_info in mapping_init.MAIN.items():
        if meta_info.get("required") and prop not in settings:
            logger.error(
                f'"Main" section does not contain the required property "{prop}"'
            )
            return False
    return True
...
def validate_settings(*, operation, settings, logger: logging.Logger):
    for prop, meta_info in operation["mapping"]["settings"].items():
        if meta_info.get("required") and prop not in settings:
            logger.error(
                f'"Settings" section does not contain the required property "{prop}"'
            )
            return False
    return True
...
def validate_iteration(
    *, operation, iteration_idx, iteration_data, logger: logging.Logger
):
...
def validate_data_settings(settings, logger: logging.Logger):
    ...
    items_def_cnt += 1 if "FIGIs" in settings else 0
    if not items_def_cnt:
        logger.error(
            '"Settings" section needs to contain one of the following properties: '
            '"P123 UIDs", "Tickers", "Cusips", "Gvkeys", "Ciks", or "FIGIs"'
        )
        return False
    if items_def_cnt > 1:
        logger.error(
            '"Settings" section can only contain one of the following properties: '
            '"P123 UIDs", "Tickers", "Cusips", "Gvkeys", "Ciks", or "FIGIs"'
        )
        return False
    ...
    elif "Ciks" in settings:
        items = settings["Ciks"]
    else:
        items = settings["FIGIs"]
    ...
    if items_cnt > 100:
        logger.error(
            '"Settings" can only contain at most 100 '
            '"P123 UIDs", "Tickers", "Cusips", "Gvkeys", "Ciks", or "FIGIs"'
        )
        return False
    return True
